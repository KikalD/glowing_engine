...
namespace lve
{
	void LveObjectSet::clear()
	{
		movingObjects.clear();
		fixObjects.clear();
	}
	void LveObjectSet::loadSet(std::string path, LveDevice &lveDevice)
	{
		clear();
		try
		{
	...
	void LveObjectSet::addObject(std::string path ,std::vector<float> floats, LveDevice &lveDevice)
	{
	...
	void LveObjectSet::addLight(std::vector<float> floats)
	{
